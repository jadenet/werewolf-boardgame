...
import { getCenterCardIds } from "./center";

function getRequiredSelections(target: Action["target"], count?: number) {
...
function getInstruction(ability: Ability, target: Action["target"], requiredSelections: number) {
  ...
  if (target === "Center" || target === "Self-Center") {
    return `Use ${ability.name}: select ${requiredSelections} center card${requiredSelections === 1 ? "" : "s"}.`;
  }
...
export function getValidTargetIds(
  ...
  let validTargetIds: Player["id"][];

  if (target === "Center" || target === "Self-Center") {
    validTargetIds = getCenterCardIds(round);
  } else if (target === "Self") {
    validTargetIds = [playerId];
  } else {
    validTargetIds = Array.from(round.playerRoles.keys());
  }
...
export async function requestAbilityPrompt(
  ...
  const requiredSelections = getRequiredSelections(action.target, action.count);
